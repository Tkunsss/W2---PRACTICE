...
function getCartTotalAmount() {
  ...
  for (let i = 0; i < SHOPPING_CART.length; i++) {
    let cart_item = SHOPPING_CART[i];
    let product = PRODUCTS.find((p) => p.id === cart_item.id);
    if (product) {
      result += product.price * cart_item.quantity;
    }
...
function addProductToCart(productId) {
  let cart_item = SHOPPING_CART.find((item) => item.id === productId);
  if (cart_item){
...
function removeProductFromCart(productId) {
  ...
  if (cart_item) {
    if (cart_item.quantity > 1) {
      ...
      SHOPPING_CART.splice(SHOPPING_CART.indexOf(cart_item), 1);
    }
  }
}
